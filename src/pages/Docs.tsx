    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-40">
        <div className="container flex items-center justify-between h-14 px-4">
          ...
          <span className="text-xs font-mono text-muted-foreground">Documentation</span>
        </div>
      </header>
      <div className="container px-4 py-8">
        <h1 className="font-heading text-2xl font-bold text-foreground mb-6">Documentation</h1>

        <div className="flex gap-1 bg-secondary rounded-lg p-1 w-fit mb-8">
          {(['traits', 'indices', 'pipeline', 'privacy'] as const).map(s => (
            <button key={s} onClick={() => setActiveSection(s)}
              className={`px-4 py-1.5 rounded-md text-xs font-heading font-medium capitalize transition-all ${activeSection === s ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
              ...
              <div key={cat}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {TRAIT_DEFINITIONS.filter(t => t.category === cat).map(trait => (
                    <div key={trait.key} className="bg-card border border-border rounded-lg p-4">
